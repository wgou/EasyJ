...
public class ConnectionPoolExcutor {
	...
	public ConnectionPoolExcutor(int corePoolSize,
		...
		if(corePoolSize < 0)
				throw new EasyJException("连接池核心连接数不能小于0");
		if(corePoolSize > maxPoolSize)
				throw new EasyJException("连接池核心连接数不能不能大于最大连接数");
		if(maxPoolSize < 0)
			throw new EasyJException("连接池最大连接数不能小于0");
		if(keepAliveTime < 0)
			throw new EasyJException("连接池连接空闲时间不能小于0");
