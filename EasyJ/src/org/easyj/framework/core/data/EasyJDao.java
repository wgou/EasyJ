package org.easyj.framework.core.data;
...
import org.easyj.framework.util.ClassMethodUtil;
import org.easyj.framework.util.EasyJException;
import org.easyj.framework.util.SQL;
 ...
 * 版	本: V1.0 
 * *************************************/
public abstract class EasyJDao {
	//构建insert sql语句
	...
	protected final  String buildDelteSQL(Class<?> clazz) throws Exception{
		...
		if(StringUtils.isEmpty(pkName)) throw new EasyJException("dont't delete not extis primarykey is bean");
	...
	protected final  String buildQueryByIdSQL(Class<?> clazz,String... columnNames) throws Exception{
		...
		if(StringUtils.isEmpty(pkName)) throw new EasyJException("dont't search not extis primarykey is bean");
